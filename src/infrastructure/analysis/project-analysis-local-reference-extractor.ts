...
type SupportedSourceLanguage = 'typescript' | 'javascript' | 'kotlin' | 'php' | 'java';
...
function buildDirectorySummaries(input: {
  ...
  return directoryPaths
    .map((path) => ({
      ...
      path,
      role: describeDirectoryRole(
        path,
        input.fileIndex.filter((entry) => entry.path.startsWith(`${path}/`)),
      ),
    }))
...
function createFileSummary(input: {
  category: string;
...
function describeLayerResponsibility(layerName: string, fileCount: number): string {
  if (layerName === 'test') {
...
function describeDirectoryRole(
  path: string,
...
function getLanguageDisplayName(language: SupportedSourceLanguage): string {
  switch (language) {
    case 'typescript':
      return 'TypeScript';
    case 'javascript':
...
function getLayerAreaDisplayName(areaName: string): string {
  switch (areaName) {
    case 'api':
      return 'API';
    case 'application':
...
function getFileCategoryDisplayName(category: string): string | null {
  switch (category) {
    case 'command':
      return '커맨드';
    case 'command-service':
