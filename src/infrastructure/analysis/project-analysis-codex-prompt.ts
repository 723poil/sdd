export function createProjectAnalysisPrompt(input: { projectName: string }): string {
  ...
  return [
    ...
    '    <document id="overview">project purpose, detected stack, entrypoints, key configs, main modules, unknowns</document>',
    '    <document id="purpose">what the product does, who it serves, important user-facing capabilities and constraints</document>',
    '    <document id="structure">top-level folders, major modules, and how responsibilities are split; use Mermaid graph or flowchart when it helps</document>',
    '    <document id="layers">logical or runtime layers, dependency direction, and ownership boundaries; use Mermaid when layer direction or boundaries are easier to see visually</document>',
    '    <document id="connectivity">important data flow, IPC, API, event links, and cross-module relationships; use Mermaid sequence or flow diagrams when useful</document>',
    '  </document_expectations>',
    '</project_analysis_request>',
  ].join('\n');
}
