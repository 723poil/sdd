export function createProjectReferenceTagGeneratorPrompt(input: {
  ...
  return [
    ...
    '  </task>',
    '  <scope>',
    '    <focus>feature or workflow cohesion across multiple technical roles</focus>',
    '    <focus>overlapping tags when one file participates in multiple real concerns</focus>',
    '    <focus>cross-cutting shared or infrastructure tags only when genuinely justified</focus>',
    '    <focus>supporting files such as mapper, helper, model, entity, factory, or utility should inherit nearby feature tags when they primarily serve one capability</focus>',
    '  </scope>',
    '  <inputs>',
    ...
    '  </inputs>',
    '  <workflow>',
    '    <stage id="analysis">Read the indexed analysis files first, inspect representative repository files when needed, and identify concrete user tasks, domain actions, bounded workflows, or subsystem responsibilities before proposing any tags. Prefer capabilities that could stand on their own as a focused implementation or review scope, and detect when the same workflow spans multiple technical roles such as controller, service, command handler, query handler, repository, job, mapper, helper, model, entity, factory, or utility. For supporting files, trace their closest callers, callees, folder neighbors, DTOs, and domain terms to decide which concrete capability they primarily enable before labeling them as shared.</stage>',
    '    <stage id="execution">Create a tag set at the feature or work-unit level and assign every indexed file to one or more tags based on actual responsibility, feature boundary, workflow involvement, or cross-cutting shared responsibility. Use overlapping tags when a file genuinely belongs to both a concrete capability and a technical or runtime concern instead of forcing a single exclusive bucket. When mapper, helper, model, entity, factory, serializer, adapter, or utility files mainly support one bounded workflow, attach them to that workflow tag even if their role sounds generic. Split broad surfaces such as admin or operations into narrower capabilities whenever the code supports it, and separate distinct lifecycle steps when the code paths are materially different.</stage>',
    '    <stage id="verification">Verify that every returned path exists in .sdd/analysis/file-index.json, every indexed file is covered by at least one tag, and no tag remains so broad that it hides distinct work scopes that should be separated. Also verify that closely related files implementing the same capability are not split apart only because they sit in different layers or CQRS-style roles. Explicitly check that mapper, helper, model, entity, factory, serializer, or utility files are not left with only a generic shared/support tag when the surrounding references indicate one dominant feature or workflow. Reject tags that are too vague to guide one coherent implementation or review task.</stage>',
    '  </workflow>',
    '  <delegation>',
    ...
    '    <constraint>Treat tags as overlapping navigation groups, not as a single exclusive partition of the repository.</constraint>',
    '    <constraint>When one file clearly participates in both a concrete capability and a cross-cutting runtime responsibility, include it in multiple tags rather than choosing only one.</constraint>',
    '    <constraint>If controller, service, command handler, query handler, repository, event handler, DTO, validator, or job files collaborate on the same user-visible capability or bounded internal workflow, give them a shared capability tag even if some of them also deserve a technical-role tag.</constraint>',
    '    <constraint>If mapper, helper, model, entity, value object, serializer, factory, adapter, transformer, or utility files mainly exist to support one concrete capability, assign them that capability tag as well instead of leaving them only under 공통, 지원, shared, support, infrastructure, model, helper, or mapper style buckets.</constraint>',
    '    <constraint>Do not treat helper, mapper, model, entity, or utility as evidence that a file is cross-cutting by default. Only use a purely shared/support tag when the file is actually reused across multiple distinct capabilities or subsystems.</constraint>',
    '    <constraint>When a supporting file sits next to a feature-specific controller, service, handler, view, command, query, or repository and shares the same domain vocabulary or references, prefer grouping it with that feature workflow unless the code shows broader reuse.</constraint>',
    '    <constraint>Do not separate a coherent workflow into isolated controller-only, service-only, or handler-only buckets when the code evidence shows they implement one capability together.</constraint>',
    ...
    '    <field_guidance field="tags">Each item represents one reusable tag group. Bias toward narrow capability tags that help someone choose a focused implementation slice, while allowing overlapping groups when the same file belongs to multiple real concerns. Include supporting mappers, helpers, models, and similar files inside capability tags when they materially enable that workflow.</field_guidance>',
    ...
    '    <field_guidance field="tags[].paths">Exact relative paths assigned to that tag. The same path may appear in multiple tag groups when justified by real responsibility overlap.</field_guidance>',
    '  </output_contract>',
    '</reference_tag_generation_request>',
  ].join('\n');
}
