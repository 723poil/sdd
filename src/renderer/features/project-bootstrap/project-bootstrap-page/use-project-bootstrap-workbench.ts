...
export function useProjectBootstrapWorkbench() {
  ...
  const canInitialize =
    inspection !== null && inspection.initializationState !== 'ready' && inspection.isWritable;
  const canAnalyze =
    inspection !== null && inspection.initializationState === 'ready' && inspection.isWritable;
  const selectedSession =
    sessions.find((session) => session.id === selectedSessionId) ?? sessions[0] ?? null;

  const storageStatus = getStorageStatus(inspection);
  ...
  const projectEntries = recentProjects;
  const analysisPreview = useMemo(
    ...
    [analysis, inspection, isAnalyzing],
  );
  ...
  async function handleReorderRecentProjects(targetRootPath: string): Promise<void> {
    ...
    const orderedRootPaths = projectEntries.map((project) => project.rootPath);
