...
flowchart LR
  ...
  node0 -->|정적 참조| node0
  node1["src/application/project/source"]
  node1 -->|정적 참조| node1
  node2["domain/domain/source"]
  node0 -->|정적 참조| node2
  node1 -->|정적 참조| node2
...
- src/application/project/source -> shared/shared/source: 정적 참조 36건. 예시: `src/application/project/activate-project.use-case.ts -> src/shared/contracts/result.ts`.
...
- 전체 파일 참조는 608건이며, 자세한 관계는 파일 인덱스와 참조 맵에서 확인할 수 있습니다.