...
flowchart TB
  ...
  project --> node4["src/contracts"]
  project --> node5["src/features/agent-cli-settings"]
  project --> node6["src/features/project-bootstrap"]
  project --> node7["src/infrastructure/agent-cli"]
```

## 핵심 디렉터리

- `.`: 주요 파일 172개. 소스 152개, 진입점 7개, 설정 파일 4개 중심 경로입니다. (main)
- `src`: 주요 파일 161개. 소스 149개, 진입점 7개, 레포지토리 4개 중심 경로입니다. (main)
- `src/application/app-settings`: 주요 파일 4개. 소스 4개 중심 경로입니다. (application)
- `src/application/project`: 주요 파일 35개. 소스 35개 중심 경로입니다. (application)
- `src/contracts`: 정적 분석에서 주요 디렉터리로 감지했습니다.
- `src/features/agent-cli-settings`: 정적 분석에서 주요 디렉터리로 감지했습니다.

## 대표 파일

- `src/main/ipc/project-ipc-registration.ts`: 메인 소스. 메인 소스입니다. TypeScript 파일 기준으로 나가는 참조 44건, 들어오는 참조 1건.
- `src/infrastructure/sdd/fs-project-storage.repository.ts`: 인프라 레포지토리. 인프라 레포지토리입니다. TypeScript 파일 기준으로 나가는 참조 14건, 들어오는 참조 2건.
