...
- 앱 전역 설정에서 CLI 에이전트 연결 정보를 관리한다
...
현재 분석 실행 경로는 이 설정을 사용해 `codex exec` 를 호출한다.

분석이 실행 중일 때는 renderer가 typed IPC로 취소를 요청할 수 있고, main process가 `codex exec` subprocess 종료와 최종 상태 정리를 담당한다.
...
4. JSONL 이벤트를 읽어 진행률을 renderer에 전달하고, 필요하면 취소 요청을 받아 subprocess 종료를 시도
5. 최종 결과를 use case가 파싱
6. 취소되지 않은 경우에만 앱이 `.sdd/analysis` 또는 `.sdd/specs`에 저장
...
- 구조화 응답은 최소한 `context.documentLinks` 와 `fileIndex[].references` 를 포함해야 하고, 앱은 이를 바탕으로 저장용 `context.fileReferences` 를 함께 집계한다
