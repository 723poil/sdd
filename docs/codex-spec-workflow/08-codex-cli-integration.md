...
현재 앱에서는 실제 분석/채팅 실행 전에 아래를 먼저 준비한다.
...
- 사용자는 PATH 자동 감지 또는 직접 실행 경로 입력을 선택하고 인증 방식을 기록할 수 있다
- `main process`에서 간단한 연결 확인을 수행할 수 있다

아직 하지 않는 것:

- 분석 실행이 이 설정을 직접 사용하지는 않는다
- 세션 채팅이 이 설정을 직접 사용하지는 않는다
- `codex exec` subprocess 실제 실행과 결과 저장은 다음 단계에서 연결한다
