...
현재 분석 실행 경로는 먼저 로컬 정적 분석으로 참조 그래프와 기본 분석 초안을 만들고, 이 설정이 가능할 때만 `codex exec` 를 호출해 문서 서술을 보강한다.

분석이 실행 중일 때는 renderer가 typed IPC로 취소를 요청할 수 있고, main process가 로컬 분석 진행 상태와 `codex exec` subprocess 종료, 최종 상태 정리를 함께 담당한다.
...
5. `전체 분석`이고 Codex 설정이 있으면 main process가 `codex exec` subprocess를 실행해 문서 서술을 보강한다
