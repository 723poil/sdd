...
- 기본 노출 문구는 제목, 현재 상태, 핵심 액션, 필요한 수치 정도로 최소화한다.
- 반복되거나 선택형인 설명은 카드 본문에 상시 노출하지 말고, 툴팁, 도움말 버튼, 접이식 도움말 같은 지연 노출로 옮긴다.
- inline 설명은 오류 복구, 빈 상태 다음 행동, 파괴적 액션 경고처럼 지금 놓치면 사용자가 막히는 정보에 우선 사용한다.
...
- UI 포맷이나 파일 경로 계산 금지
- 수동 reference-tag/assignment 편집은 use case가 저장 책임을 갖고 renderer local-only state에 남기지 않는다
